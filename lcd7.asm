...
IRD1    bsf     porta,1         ; turn on LED
        ...
        btfsc   rf,7            ; if bit 7 of byte pointed to by fsr
        movlw   IRDone          ; is set, then use delay for "one"
        bcf     porta,1         ; turn off LED
        call    IRwait
        rlf     rf,f            ; prepare next bit
...
IRw2    decfsz  re,f            ; 1/2   |              92 |
        goto    IRw2            ; 2     | 16*3=90+2=92    |100*W
        decfsz  rd,f            ; 1/2                 1/2 |
        goto    IRw1            ; 2                     2 |
        return                  ; 2                     2 |

RomDec  decf    romptr,f        ; decrement ROM pointer
        btfss	romptr,7	; see if decremented past 0
	goto    RomPrt		; no, fetch data
	movlw	ROM_MAX		; yes, set ROM pointer to highest ROM location
	movwf	romptr
	goto	RomPrt

RomInc  incf    romptr,f        ; increment ROM pointer
	movlw	ROM_MAX + 1
	subwf	romptr,W
	btfss	status,C
	goto	RomPrt
